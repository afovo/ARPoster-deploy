...
export default function Mercer(data) {
    // -----------------------------------------------
    ...
    var Graph = function(container, graphName, graphData, rowCollection) {
        ...
        var calculateCamera = function(cameraSettings, graphObjectArea) {
            ...
            graphObjectArea.getSize(size)
            var maxDimension = Math.max(size.x, size.y);
    ...
    RowLabel.prototype.draw = function() {
        var textGeometry = new TextGeometry(this._text, {
            font: _font,
            size: this._size,
            height: .2
        });
        ...
        this.textMesh.position.z = ((this._row*this._rowSpace) + (this._row*this._rowWidth) + (this._rowWidth/2) + (size.z/2));
    ...
    var _fontLocation = "/fonts/gentilis_bold.typeface.json";
    var _font = null;
    ...
    return {
        ...
        LineGraph: function(container, graphData, loaded) {
            ...
            Row.prototype.draw = function(graphMinX, graphMinY, graphMinZ) {    
                ...
                var vertices = []
                for (var i=0; i<this._linePoints.length; i++) {
                    vertices.push(this._linePoints[i].getX(), this._linePoints[i].getY()-graphMinY, (this._lineWidth/2));
                }
        ...
        AreaChart: function(container, graphData, loaded) {
            ...
            Row.prototype.draw = function(graphMinX, graphMinY, graphMinZ) {    
                ...
                var areaGeometry = new THREE.BufferGeometry();
                ...
                // Generate the outline
                var areaLineGeometry = new THREE.BufferGeometry();
                for (var i=0; i<this._areaPoints.length; i++) {
